...
parser.add_argument(
    "-n", "--names", type=int, default=None, help="Number of unique names"
)
parser.add_argument("-s", "--size", type=int, default=None, help="Average object size")
parser.add_argument(
    "-e", "--seed", type=int, default=None, help="Random generator seed"
)
parser.add_argument(
    "-r",
    "--requests",
...
parser.add_argument(
    "-f", "--rpf", type=int, default=1000000, help="Number of requests per file"
)
...
for i in range(0, requests, args.rpf):
    ...
    timestamps = np.random.exponential(0.5, size=reqs_to_generate).astype(np.int64)
