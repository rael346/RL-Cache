...
from environment.environment_aux import class2name, load_json, name2class
...
def check_existance(
    name, value, exists, tabulation, level, verbose, dead, directory=False
):
    if not directory:
...
def apply_config(configuration_rules, configuration, tabulation):
    for key in configuration.keys():
        ...
        if rules["interval"] is not None:
            l, r = rules["interval"]
            if not check_range(
                key, configuration[key], l, r, tabulation, 0, False, True
            ):
                return None
    ...
    keys_to_fill = [
        key for key in configuration_rules.keys() if key not in configuration.keys()
    ]
...
def compare_statistics_dicts(d1, d2):
    ...
    if (
        "load" not in c1.keys()
        or "save" not in c1.keys()
        or "show stat" not in c1.keys()
    ):
        return False
    if (
        "load" not in c2.keys()
        or "save" not in c2.keys()
        or "show stat" not in c2.keys()
    ):
        return False
    c1["load"] = c2["load"]
    c1["save"] = c2["save"]
...
def load_caching_algorithms(algorithms, tabulation, verbose):
    ...
    for key in algorithms.keys():
        ...
        if experiment is not None:
            ...
            model_config = check_model_config(
                experiment, tabulation=tabulation, verbose=verbose
            )
    ...
    return (
        featurers,
        known_admission_models,
        known_eviction_models,
        models,
        common_models_mapping,
    )
...
def check_statistics_config(experiment_name, tabulation="", verbose=True):
    ...
    if not config["save"] and not config["load"]:
        print(
            tabulation,
            error_levels[1],
            "We would strongly recommend you to save or load the data",
        )
    ...
    if not os.path.isfile(config["filename"]) and config["load"]:
        print(
            ...
            "File",
            config["filename"],
            "does not exists, data will be generated",
        )
...
def check_model_config(experiment_name, tabulation="", verbose=True):
    ...
    if not check_existance(
        "statistics", config["statistics"], True, tabulation, 0, verbose, True
    ):
        return None
...
def check_train_config(experiment_name, tabulation="", verbose=True):
    ...
    config["model"] = check_model_config(
        experiment_name, tabulation=tabulation, verbose=verbose
    )
    if config["model"] is None:
        return None

    config["feature extractor"] = check_statistics_config(
        experiment_name, tabulation, verbose
    )
    if config["feature extractor"] is None:
        return None

    config["session configuration"] = check_session_configuration(
        config["session configuration"], tabulation
    )
